...
class CoursesController < ApplicationController
  ...
  def index
    @courses = if !params[:search].nil?
                 Course.search(params[:search]).where(visibility: 1)
               else
                 Course.all.where(visibility: 1)
               end
  end
  ...
  def destroy
    ...
    respond_to do |format|
      if @course.lessons.size.positive?
        ...
        format.html do
          redirect_to course_url(@course), alert: 'No puedes hacer publico un curso que no tiene lecciones'
        end
